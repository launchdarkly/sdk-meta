...
1. Use a runtime-appropriate base through a pre-`FROM` build argument. Base tags live in `validators/shared/versions/images.env`; declare the matching `ARG` with no inline default before `FROM ${ARG}`. The validator CLI supplies the value automatically, and `validators/build.sh <runtime>` is the supported equivalent for human Docker builds. Node bases are on the 22 line or newer because npm 12 — required by the install quarantine below — declares `engines.node: ^22.22.2 || ^24.15.0 || >=26.0.0`.
   - Keep pins split by category: npm package and package-manager pins live in `shared/versions/npm.env`, non-npm toolchain/download pins live in `shared/versions/toolchains.env`, and image tags live in `shared/versions/images.env`. Copy and source the relevant env file in the image rather than duplicating values in Dockerfiles. For Playwright validators, the image tag and `PLAYWRIGHT_VERSION` npm pin move together.
   - If the image installs npm packages at build time or from a harness, source `npm.env`, install npm 12, and write `/root/.npmrc` with `min-release-age=3` plus `min-release-age-exclude[]=@launchdarkly/*`, `min-release-age-exclude[]=launchdarkly-*`, and `min-release-age-exclude[]=highlight.run`. Third-party packages are held for three days; LaunchDarkly packages stay installable the moment they publish, which is the point of these validators. `highlight.run` is LaunchDarkly-owned too: `@launchdarkly/observability` and `@launchdarkly/session-replay` pin it to an exact version from the same release train, so it must bypass the cooldown or a fresh observability release fails to install for three days. The config is user-level, not project-level, because harnesses install inside a `mktemp` workdir. Repeated `.npmrc` keys overwrite each other — the `[]=` array syntax is load-bearing.
...
3. Layer additional toolchains as needed. `shell-install` builds on the centralized `NODE_BOOKWORM_SLIM_IMAGE`, adds Go via the upstream tarball, pip via `python3-pip`, and sources `toolchains.env` for the Go and Gradle pins. It sources `npm.env` for npm 12, `corepack prepare pnpm@11 --activate`, and the classic `yarn@1` pin. The 22 line is deliberate: it still bundles corepack (Node 25 drops it), and Node 20 is below the `engines.node` floor that `@napi-rs/lzma` — pulled in transitively by the Fastly SDK — enforces on yarn installs. pnpm is pinned to 11 for the quarantine: `minimumReleaseAge` landed in 10.16, and 11 reads the documented `/root/.config/pnpm/config.yaml` (4320 minutes, with `minimumReleaseAgeExclude` for the LaunchDarkly globs). yarn stays on the classic line every install snippet uses, which has no age gate — that path is ungated.
...
Build context is `validators/` (see the comment in `python/Dockerfile`), which is why paths are `shared/` and `languages/<runtime>/...`. Centralize pins in `shared/versions/npm.env`, `shared/versions/images.env`, or `shared/versions/toolchains.env`; the CLI passes only values for `ARG`s declared by the selected Dockerfile.
