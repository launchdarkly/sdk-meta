...
1. Use a runtime-appropriate base through a pre-`FROM` build argument. Base tags live in `validators/shared/versions/images.env`; declare the matching `ARG` with no inline default before `FROM ${ARG}`. The validator CLI supplies the value automatically, and `validators/build.sh <runtime>` is the supported equivalent for human Docker builds. Node bases are on the 22 line or newer because npm 12 — required by the install quarantine below — declares `engines.node: ^22.22.2 || ^24.15.0 || >=26.0.0`.
   ...
   - If the image installs npm packages at build time or from a harness, source `npm.env`, install npm 12, and write `/root/.npmrc` with `min-release-age=3` plus `min-release-age-exclude[]=@launchdarkly/*` and `min-release-age-exclude[]=launchdarkly-*`. Third-party packages are held for three days; LaunchDarkly packages stay installable the moment they publish, which is the point of these validators. The config is user-level, not project-level, because harnesses install inside a `mktemp` workdir. Repeated `.npmrc` keys overwrite each other — the `[]=` array syntax is load-bearing.
