...
1. `FROM` a runtime-appropriate base (`python:3.11-slim`, `node:22-bookworm-slim`, `node:22-bookworm`, `node:22`, `mcr.microsoft.com/playwright:v1.60.0-noble`, `eclipse-temurin:17-jdk-noble`). Node bases are on the 22 line or newer because npm 12 — required by the install quarantine below — declares `engines.node: ^22.22.2 || ^24.15.0 || >=26.0.0`.
   - If the image installs npm packages at build time or from a harness, `npm install -g npm@12` and write `/root/.npmrc` with `min-release-age=3` plus `min-release-age-exclude[]=@launchdarkly/*` and `min-release-age-exclude[]=launchdarkly-*`. Third-party packages are held for three days; LaunchDarkly packages stay installable the moment they publish, which is the point of these validators. The config is user-level, not project-level, because harnesses install inside a `mktemp` workdir. Repeated `.npmrc` keys overwrite each other — the `[]=` array syntax is load-bearing.
