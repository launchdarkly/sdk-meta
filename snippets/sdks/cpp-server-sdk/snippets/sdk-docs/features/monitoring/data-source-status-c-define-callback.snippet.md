---
id: cpp-server-sdk/sdk-docs/features/monitoring/data-source-status-c-define-callback
sdk: cpp-server-sdk
kind: reference
lang: c
description: Data source status callback definition for C++ (server-side) SDK v3.0 (C binding).
validation:
  scaffold: cpp-server-sdk/scaffolds/cpp-syntax-only-toplevel

---

```c
void OnDataSourceStatusChanged(LDServerDataSourceStatus status, void* user_data) {
  printf("status: %d\n", LDServerDataSourceStatus_GetState(status));
}
```
